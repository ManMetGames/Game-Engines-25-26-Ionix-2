...
function ExampleScript:OnUpdate()
    ...
    if coinActive and Mafs.get_vec_x(coinPos) < pipeOffScreenLeft then
    ...
    function ExampleScript:OnTriggerEnter(collision1, collision2)
        if (collision1 == player1 and collision2 == coin) or (collision1 == coin and collision2 == player1) then
            if coinActive then
                print("TriggerCoin")
                Fysics.set_pos(coin, -1000, 300)
                Fysics.set_linear_velocity(coin, 0, 0)
                coinActive = false
            end
        end
end
